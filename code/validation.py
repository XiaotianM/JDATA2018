...
Data = DataLoader(
				...
				startMonth = datetime(2017, 4, 1),
				endMonth = datetime(2017, 6, 30)
			)
# train data
TrainFeatures = Features(
						DataLoader=Data,
						PredMonthBegin = datetime(2017, 7, 1),
						PredMonthEnd = datetime(2017, 7, 30),
						FeatureMonthList = [
									(datetime(2017, 6, 30)-timedelta(7)*1+timedelta(1),   datetime(2017, 6, 30), 0.25),\
									(datetime(2017, 6, 30)-timedelta(7)*2+timedelta(1),   datetime(2017, 6, 30), 0.5),\
									(datetime(2017, 6, 30)-timedelta(7)*3+timedelta(1),   datetime(2017, 6, 30), 0.7),\
									(datetime(2017, 6, 30)-timedelta(30)*1+timedelta(1),  datetime(2017, 6, 30), 1),\
									(datetime(2017, 6, 30)-timedelta(30)*3+timedelta(1),  datetime(2017, 6, 30), 3),\
									(datetime(2017, 6, 30)-timedelta(30)*6+timedelta(1),  datetime(2017, 6, 30), 6),\
									(datetime(2017, 6, 30)-timedelta(30)*9+timedelta(1),  datetime(2017, 6, 30), 9),\
									(datetime(2017, 6, 30)-timedelta(30)*11+timedelta(1), datetime(2017, 6, 30), 11)], # begin:2016,05,06
						MakeLabel = True
					)
...
PredFeatures = Features(
					...
					FeatureMonthList = [
									...
									(datetime(2017, 7, 31)-timedelta(30)*11+timedelta(1), datetime(2017, 7, 31), 11)], # begin:2016,06,05
					MakeLabel = True   # for validation
				)
# 8月测试
...
cls_model = SBBTree(params=params, stacking_num=5, bagging_num=3,  bagging_test_size=0.33, num_boost_round=10000, early_stopping_rounds=100)
...
cls_model.fit(X,y)
Offline_Pred[train_label_BuyNum] = cls_model.predict(X_pred)  # 线下 S1
...
reg_model = SBBTree(params=params, stacking_num=5, bagging_num=3,  bagging_test_size=0.33, num_boost_round=10000, early_stopping_rounds=100)
...
reg_model.fit(X,y)
Offline_Pred[train_label_FirstTime] = reg_model.predict(X_pred)
...
real = PredFeatures.data_BuyOrNot_FirstTime[PredFeatures.data_BuyOrNot_FirstTime["Label_30_101_BuyNum"]>0][['user_id']+[train_label_FirstTime]].\
...
# 				)
# X_pred = PredFeatures.data_BuyOrNot_FirstTime[train_features].values 
# PredFeatures.data_BuyOrNot_FirstTime['Label_30_101_BuyNum'] = cls_model.predict(X_pred)
# PredFeatures.data_BuyOrNot_FirstTime['Label_30_101_FirstTime'] = reg_model.predict(X_pred)
...
# out_submit.head(50000).to_csv('../submit/predict_validation.csv',index=False,header=True)